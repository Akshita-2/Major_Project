...
from components.ui_utils import display_star_method_guide
...
def display_structured_feedback(feedback):
    ...
    # Use regex to find the score, defaulting to 0 if not found
    score_match = re.search(r"Overall Score:.*?(\d+)/10", feedback, re.IGNORECASE)
    score = int(score_match.group(1)) if score_match else 0
    
    # Extract sections using split logic
    try:
        ...
        st.markdown(feedback) # Fallback for unexpected formats
        return
...
def page_interview_prep():
    ...
    st.markdown("Practice with tailored questions and get instant feedback from your AI career coach.")
    ...
    if not st.session_state.get('resume_data') or not st.session_state.get('job_description'):
        st.info("👈 Please input your resume and a job description to get personalized interview questions.")
        return

    if st.button("🤖 Generate AI Interview Questions", type="primary"):
        with st.spinner("🧠 Gemini is crafting questions..."):
            ai_helper = GeminiAIHelper(st.session_state.gemini_model)
            questions = ai_helper.generate_interview_questions(st.session_state.job_description, st.session_state.resume_data)
            ...
            if questions:
                ...
                st.error("The AI could not generate questions. Please try again.")

    st.markdown("---")
    
    questions = st.session_state.get('interview_questions', [])
    if not questions:
        st.info("Click the button above to generate personalized interview questions.")
        return
    ...
    with tab2:
        ...
        if not filtered_questions:
            st.warning(f"No questions found for the category '{filter_category}'. Please select another.")
            return
        ...
        if q and isinstance(q, dict):
            ...
            # NEW FEATURE: Voice or Text Input
                ...
                if st.button("🤖 Get AI Feedback", type="primary"):
                    if st.session_state.user_answer:
                        with st.spinner("🤖 Evaluating your answer..."):
                            ai_helper = GeminiAIHelper(st.session_state.gemini_model)
                            feedback = ai_helper.evaluate_interview_answer(q.get('question'), st.session_state.user_answer, st.session_state.job_description)
            ...
            if st.session_state.get('last_feedback'):
                st.markdown("---")
                # NEW FEATURE: Structured Feedback Display
